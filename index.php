 ...
 *
 * Amaç:
 * - Kullanıcının e-posta/şifre ile oturum açmasını sağlamak
 * - Doğrulama başarılıysa rolüne göre ilgili panele yönlendirmek
 *
 * Not: Yalnızca açıklama eklendi, mevcut davranış korunmuştur.
 */

// Oturumu başlatır; giriş başarılı olursa kullanıcı bilgileri $_SESSION içine yazılır
session_start();

// Veritabanı bağlantısı
include 'config.php';

// Kullanıcıya gösterilecek bilgilendirme/hata mesajı için değişken
$message = "";

// Form gönderimi kontrolü (sadece POST olduğunda çalışır)
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // E-posta ve şifre değerlerini al, baştaki/sondaki boşlukları kırp
    ...
    if ($email !== '' && $password !== '') {
        ...
        if ($stmt->num_rows == 1) {
            // Kullanıcı bulundu, şifre ve durum kontrol edilecek
            $stmt->bind_result($user_id, $hashed_password, $role, $status);
            $stmt->fetch();

            // password_verify: girilen şifre ile veritabanındaki hash'i karşılaştırır
            if (password_verify($password, $hashed_password)) {
                // Sadece onaylı hesaplar giriş yapabilir
                if ($status === 'approved') {
                    // Oturum bilgileri: sonraki sayfalarda rol bazlı erişim için kullanılır
                    $_SESSION['user_id'] = $user_id;
                    $_SESSION['email'] = $email;
                    $_SESSION['role'] = $role;

                    // Son giriş zamanını güncelle (istatistik/denetim için faydalı)
                    $ul = $conn->prepare("UPDATE users SET last_login = NOW() WHERE id = ?");
                    $ul->bind_param("i", $user_id);
                    $ul->execute();
                    $ul->close();
                    ...
                    // Hesap henüz admin tarafından onaylanmadı
                    $message = "<div class='error'>Hesabınız henüz onaylanmamış. Lütfen admin onayını bekleyin.</div>";
                }
            } else {
                // Şifre eşleşmedi
                $message = "<div class='error'>Şifre hatalı.</div>";
            }
        } else {
            // E-postaya ait kullanıcı bulunamadı
            $message = "<div class='error'>Bu e-posta ile kayıtlı bir kullanıcı bulunamadı.</div>";
        }
        ...
        $message = "<div class='error'>E-posta ve şifre zorunludur.</div>";
    }
}
?>
...
<head>
    ...
    <title>Eğitim Sistemi - Giriş</title>
    <script>
    ...
    <div class="navbar-nav">
      ...
      <?php else: ?>
        ...
        <a class="nav-button" href="register.php">
          <i class="fas fa-user-plus" style="padding-top: 10px;"></i>
          Kayıt Ol
        </a>
      <?php endif; ?>
    </div>
...
<div class="main-content">
    ...
    <div class="logo-section">
        ...
        <h1 class="logo-text">Eğitim Sistemi</h1>
        <p class="subtitle">Bilgiye açılan kapınız</p>
    </div>
    
    <!-- Giriş formu: e-posta ve şifre alanları zorunludur -->
    <form method="POST" action="" autocomplete="on">
        <div class="form-group">
            <i class="fas fa-envelope"></i>
            <input type="email" name="email" class="form-input" placeholder="E-posta adresiniz" required autocomplete="email">
        </div>
    ...
    <div class="nav-link">
        Hesabınız yok mu? <a href="register.php" class="kayit_ol">Kayıt Ol</a>
    </div>
